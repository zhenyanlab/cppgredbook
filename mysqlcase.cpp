...
int structDataToJsonString(vector<person_t> & db_p_list, char *pjd)
{
    try
    {
        Json::Value root;
        ...
        std::string jsonstr= arrayObj.toStyledString();
        strcpy(pjd, jsonstr.c_str());
    ...
    for (int k = 0; k < 1; k++)
    {
        ...
        if (retcode == 0)
        {
            for (int i = 0; i < db_p_list.size(); i++)
            {
                ...
                std::cout << p.user_id << "\t\t" << db_p_list[i].id << "\t\t"
                          << "|" << namess << "|" << std::endl;
                //   error
                //   delete p.name;
            }
    ...
    WFHttpServer server([](WFHttpTask *task)
                        {
                            ...
                            retcode = get_all_p_list(&db_p_list);
                            char respstr [10240] ;
                            memset(respstr,65,10240);
                            if (retcode == 0)
                            {
                                structDataToJsonString(db_p_list,respstr);
                                printf("jsoncpp:%llu\n",strlen(respstr));
                            }

                            task->get_resp()->append_output_body(respstr);
                            //--------------------------------------------//
